...
def load_birefnet_model(model_path, device):
    ...
    """
    # 检测模型类型
    is_lite_model = 'lite' in os.path.basename(model_path).lower()
    ...
    if bb_weight_shape:
        is_lite_based_on_shape = bb_weight_shape[0] == 96  # lite版是[96,3,4,4]，完整版是[192,3,4,4]
        print(f"根据参数形状推断: {'Lite版' if is_lite_based_on_shape else '完整版'} (嵌入层大小: {bb_weight_shape[0]})")
    
    # 根据推断结果选择模型配置
    if is_lite_based_on_shape:
        # lite版模型
        ...
        # 使用自定义函数加载完整版模型
        print("加载完整版BiRefNet模型 - 此功能需要额外实现")
        print("注意: 当前脚本并未完全实现完整版模型的加载")
        print("将尝试使用非严格加载方式加载模型，可能会导致性能下降")
...
if __name__ == "__main__":
    # 视频路径
    video_path = "/root/gpufree-data/111.mp4"
    model_path = "BiRefNet_lite-general-2K-epoch_232.pth"
    
    # 输出路径
    video_name = os.path.splitext(os.path.basename(video_path))[0]
    output_mask_path = os.path.join(os.path.dirname(video_path), f"{video_name}-mask-gpu.mp4")
    output_composite_path = os.path.join(os.path.dirname(video_path), f"{video_name}-composite-gpu.mp4")
    
    # 设置批处理大小 - 根据GPU内存大小调整
    if torch.cuda.is_available():
        # 计算合适的批处理大小 - 4090可用24GB，相应调整
        gpu_mem = torch.cuda.get_device_properties(0).total_memory / 1e9
        if gpu_mem > 20:  # 高端GPU (4090, A100等)
            batch_size = 8  # 2K分辨率下减小批大小
    ...
    # 设置输入分辨率 - 较大的分辨率提供更好的质量，但需要更多内存
    image_size = (2560, 1440)  # 使用2K分辨率以获得最佳质量
    print(f"使用2K分辨率处理: {image_size} - 注意此设置需要较大GPU内存")
